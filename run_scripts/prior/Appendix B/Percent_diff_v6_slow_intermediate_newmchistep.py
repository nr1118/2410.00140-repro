    ...
    for i,e in enumerate(central_densities):
        ...
        if Radius_0!=0 and Radius_4!=0: #(Testing for an ADM core because if there is an ADM halo then R_star = 0)
            if Mass_0 >=1. and Mass_4 >=1.:
                ...
                #like Mass_0 = 1.663 Mass_4 = 1.665, which could be treated as the same mass since they differ by 0.0025 solar mass (~0.1%). So, I placed an additional condition that checks
...
numpy.save('Relcent_diff40_intermediately_stiff_baryonic_newmchistep.npy',Array)
...
ax.set_title('Intermediately Stiff EoS with $\Delta m_\chi$ = 100 MeV')
ax.set_ylabel('$F_\chi \, (\%)$')
ax.set_xlabel('$m_\chi \, (MeV)$')
pyplot.savefig('Percent_diff_intermediate_stiff_plot_newmchistep.png')
pyplot.show()
